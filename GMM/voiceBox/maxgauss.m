...
%      Version: $Id: maxgauss.m,v 1.1 2007/07/25 15:11:33 dmb Exp $
%
...
p=eye(nm);
ix=find(m==-Inf);       % remove negaive infinities immediately
if ~isempty(ix)
    m(ix)=[];
    c(ix,:)=[];
    c(:,ix)=[];
...
ix=0:nm^2-1;        % index for upper triangular matrix elements (from 0)
ix = ix(ix<(nm+1)*floor(ix/nm));
...
while nm>1
    ...
    if ~isempty(jx)               % special case: two variables differ by a constant
        ...
        dms=dm/ds;
        % q=normcdf(dm,0,ds)
        q = 0.5 * erfc(nrh*dms);             % prob x(i) > x(j)
        % f=ds*normpdf(dm,0,ds);
        f=kpd*exp(-0.5*dms^2);
        mij(1)=m(i);
        mij(2)=m(j);
        u=dm*q+mij(2)+ds*f;             % mean of max{x(i),x(j)}
        v=(mij(1)+mij(2)-u)*u +cd(i)*q+cd(j)*(1-q)-mij(1)*mij(2); % variance of max
